...
|---|---|---|
| `helpers.maneuver_node` | `execute_node(state, commands, node, staging_mode, dt, log, pointing=None)` | Drive the vessel through a kRPC maneuver node. Calls `auto_stage` internally when `staging_mode` is not `None`. Steers via the `pointing` `PointingController` (param default `auto`: KSP's native SAS maneuver-hold, falling back to the kRPC autopilot when the mode never latches); `pointing=None` steers with the autopilot. Holds the throttle at 0 until the vessel is aligned with the burn vector (within 10 deg), so it never fires while still slewing. Tapers throttle across the last ~8 ticks of the burn (using `dt` and the node's `burn_time_estimate`) so a high-TWR upper stage does not overshoot in the last tick. Returns `True` when the burn completes. |
| `helpers.maneuver_node` | `POINTING_PARAM` / `parse_pointing(value)` | Canonical `pointing` `ActionParam` for node-driven actions plus its parser (`str | None -> PointingController`, default `auto`). Add the param to `params`, parse in `start()`, pass to `execute_node`. Strict `sas_maneuver` never falls back: without a maneuver-capable pilot/probe core the burn holds at zero throttle. |
