...
def _format_orbit(state: VesselState) -> str:
    return "\n".join(
        [
            ...
            "",
            "[b]Control Inputs[/b]",
            f"Pitch input:     {state.input_pitch:+.2f}",
            f"Yaw input:       {state.input_yaw:+.2f}",
            f"Roll input:      {state.input_roll:+.2f}",
            "",
            "[b]Propulsion[/b]",
...
def _format_resources(state: VesselState) -> str:
    return "\n".join(
        [
            ...
            f"SAS mode:        {state.sas_mode.display_name if state.sas_mode is not None else '-'}",
