...
- `ControlScreen` (~148 lines) owned the kRPC connection, `ActionRunner`, `threading.Event`, the poll loop, demo mode branching, and shutdown/abort logic.
...
- The blocking poll loop (`run_poll_loop()`) and demo tick (`demo_tick()`)
...
- **Positive**: Demo vs live mode is handled inside `ControlSession`, invisible to the screen.
