...
- **kRPC frame conversions ignore future time**: `orbit.position_at(ut, body.reference_frame)` converts through the body's rotation **at call time**, not at `ut` (`ReferenceFrame.PositionFromWorldSpace` takes no time parameter). Any future position mapped to lat/lon must be rotated back by `360 / rotational_period * (ut - now)` degrees of longitude. The impact predictor in `krpc_bridge.py` does this; copy that pattern for any new future-position query.
