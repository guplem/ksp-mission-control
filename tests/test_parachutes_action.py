...
class TestParachutesActionTick:
    ...
    def test_deploys_at_high_altitude(self) -> None:
        action = self._make_started_action()
        state = State(
    ...
    def test_stages_when_no_parachutes_in_current_stage(self) -> None:
        action = self._make_started_action(stage_for_parachutes=True)
        state = State(
    ...
    def test_fails_when_not_in_stage_and_staging_disabled(self) -> None:
        action = self._make_started_action(stage_for_parachutes=False)
        state = State(
            altitude_surface=1000.0,
            stage_current=5,
            parts=Parts(parachutes=(ParachuteInfo(3, "stowed"),)),
    ...
    def test_waits_when_unsafe_and_wait_for_safe_enabled(self) -> None:
        action = self._make_started_action(wait_for_safe=True)
        state = State(
            altitude_surface=1000.0,
            stage_current=3,
    ...
    def test_deploys_regardless_when_wait_for_safe_disabled(self) -> None:
        action = self._make_started_action(wait_for_safe=False)
        state = State(
            altitude_surface=1000.0,
            stage_current=3,
            parts=Parts(parachutes=(ParachuteInfo(3, "stowed", safe_to_deploy=False),)),
